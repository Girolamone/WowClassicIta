...
function WowClassicIta:OnEnable()
    ...
    if not questFrameIdButton or not questLogIdButton or not gossipIdButton then
        ...
        return
    elseif (not questFrameIdButton or not questLogIdButton) and self.db.profile.quests.enabled and not self.db.profile.disabled then
        self:Error("Impossible to create the quest/questlog frame button!")
        self:Error("Extension cannot be loaded, so will be disabled.")
        self:Error("If you want, open an issue at: " .. addonTable.GitHubURL)
        self:UnregisterAllEvents()
        return
    elseif not gossipIdButton and self.db.profile.gossips.enable and not self.db.profile.disabled then
    ...
    self:RegisterEvent('PLAYER_LOGIN', function()
        ...
        self:Debug("Current logged user information:")
        self:Debug("Player name: " .. self.characterName)
        self:Debug("Player class: " .. self.characterClass)
        self:Debug("Player race: " .. self.characterRace)
        self:Debug("Player sex: " .. self.characterSex)
    end)
    ...
    self:RegisterEvent("GOSSIP_SHOW", function()
        if not self.db.profile.gossips.enabled then
